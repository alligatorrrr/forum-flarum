...
app.initializers.add('clarkwinkelmann-post-stream-search', () => {
    //只看楼主功能
    var textOnlyStarter = "楼主";
    var textNotOnlyStarter = "取消";
    extend(PostStream.prototype, 'oninit', function () {
        onlyStarter = false;
    });
    extend(PostStream.prototype, 'oninit', function () {
        onlyStarter = false;
        if (this.attrs.discussion && this.attrs.discussion.firstPost()) {
            postAuthor = this.attrs.discussion.firstPost().user();
        }
    });
            ...
            Button.component(
                {
                    ...
                    type: 'button',
                    onclick: () => {
                        if (!postAuthor.id) return;
                        if (!onlyStarter) {
                            const discussion = this.attrs.discussion;
                            //只看楼主
                            const stream = app.current.get('stream');
                            if (stream) {
                                stream.filterUsers = [postAuthor];
                                stream.applyFilters();
                            }
                            onlyStarter = true;
                ...
                if (post && post.discussion() && typeof post.canEdit() !== 'undefined') {
                    if (!this.filterSearch) {
                        if (post["data"]["attributes"]["contentHtml"]) {
                            post["data"]["attributes"]["contentHtml"] = post["data"]["attributes"]["contentHtml"].replaceAll("<mark>", "").replaceAll("</mark>", "")
                        }
                    }
                    loaded.push(post);
                } else {
        ...
        this.retrieveFilteredDiscussion(near).then(payload => {
            ...
            payload.data.forEach(entry => {
                ...
                if (entry.attributes) {
                    ...
                    if (post["data"]["attributes"]["contentHtml"]) {
                        if (this.filterSearch) {
                            let templateArray = post["data"]["attributes"]["contentHtml"].replaceAll("<mark>", "").replaceAll("</mark>", "").split(/(<[^>]+>)/g);
                            templateArray.forEach((item, index) => {
                                if (item.search('<') > -1) {
                                    return;
                                }
                                templateArray[index] = item.replace(new RegExp(`(${this.filterSearch})`, 'ig'), "<mark>" + this.filterSearch + "</mark>");
                            })
                            post["data"]["attributes"]["contentHtml"] = templateArray.join('');
                        }
                    } else {
                        if (post["data"]["attributes"]["contentHtml"]) {
                            post["data"]["attributes"]["contentHtml"] = post["data"]["attributes"]["contentHtml"].replaceAll("<mark>", "").replaceAll("</mark>", "")
    ...
    extend(CommentPost.prototype, 'headerItems', function (items) {
        ...
        if (user && user === discussion.user()) {
            ...
            }, app.translator.trans('clarkwinkelmann-post-stream-search.forum.post.opBadge'))), 100)
        }
    });
});
