...
export default class MobileTab extends Component {
  ...
  items(): ItemList<Children> {
    ...
    if (app.routes.index.path === '/all') {
      ...
      items.add(
        'categories',
        <MobileTabItem route={app.route('categories')} icon="fas fa-tags" label={app.translator.trans('acpl-mobile-tab.forum.categories')} />,
        90
      );
 ...
 items.add('refresh', <LinkButton icon="fas fa-refresh"  className="Button Button--link" label='刷新' onclick={() => {if (app.session.user) {
