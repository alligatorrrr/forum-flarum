...
WaterMark.prototype = {
  ...
  _init: function(node, options){
    this.options = {
      node:node,                                               // 添加水印的节点
      text: options.text ? options.text + " 小狗梦工厂" : "小狗梦工厂",                      // 水印文字内容
      opacity: options.opacity?options.opacity:0.05,           // 水印透明度
      startX: options.startX?options.startX:20,                 // X轴开始位置
      startY: options.startY?options.startY:20,                // Y轴开始位置
      xSpace: 20,                                              // 横向间隔
      ySpace: 40,                                              // 纵向间隔
      rows: 0,                                                 // 行数
      cols:0,                                                  // 列数
      markWidth: options.markWidth?options.markWidth:880,      // 水印高度
      markHeight: options.markHeight?options.markHeight:580,    // 水印宽度
      angle: options.angle?options.angle:15,                   // 倾斜角度
      fontSize: options.fontSize?options.fontSize:30,          // 字体大小
      color: options.color?options.color:"#666",               // 字体颜色
      fontFamily: options.fontFamily?options.fontFamily: 'Arial, sans-serif', // 字体
  ...
  _fillContent:function(){
    ...
    if(!this.markContainer) {
      ...
      this.markContainer.className = 'watermark-layer'; // 添加类名
    }
    
    this.markContainer.style.position = "static";
  ...
  _createWaterMark: function(x, y){
    ...
    if(this.options.imageSrc){
      ...
      markDiv.style.mixBlendMode = "difference";
    }
    //设置水印div倾斜显示
    ...
    markDiv.style.fontSize = this.options.fontSize + 'px';
  ...
  /* 刷新水印 水印容器大小发生变化是调用 */
  refresh: function() {
    if(this.markContainer.style.display == 'none') return;
    if(this.markContainer){
